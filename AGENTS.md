...
| 사용자 호출 전용 스킬 (`grill` · `spec` · `step` · `reply-review` · `like-gpt` · `autoprompt`) | `disable-model-invocation: true` 가 있는가 · `$ARGUMENTS` 가 비어 있을 때의 동작이 정의됐는가 (`when_to_use` 는 불필요 — 리스팅에 안 들어간다) |
